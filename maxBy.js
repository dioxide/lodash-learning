 ...
 * 此方法类似于`max`但其接受一个应用在`array`的每个元素上以生成要被rank的值的`iteratee`函数。
 * 迭代器接受1个参数：（value）
 *
 * @since 4.0.0
 * @category Math
 ...
 * @returns {*} Returns the maximum value. 最大值
 * @example
 *
...
function maxBy(array, iteratee) {
  ...
  for (const value of array) {  // 迭代数组
    ...
    // 只有在 迭代后的当前值不是null 且 （记录值是undefined时current自等且不是symbol 或 迭代后的当前值大于记录值）
